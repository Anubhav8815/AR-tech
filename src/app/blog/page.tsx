  ...
  return (
    <main className="min-h-screen bg-[#030712] px-6 py-24 text-slate-100 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <p className="text-sm font-semibold uppercase tracking-[0.3em] text-emerald-400">Blog</p>
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {posts.map((post) => (
            <article key={post.title} className="rounded-[2rem] border border-white/10 bg-white/10 p-8 shadow-2xl shadow-black/20 backdrop-blur">
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-amber-300">{post.category}</p>
        ...
        <Link href="/" className="mt-10 inline-flex rounded-full bg-gradient-to-r from-emerald-500 to-amber-500 px-6 py-3 font-semibold text-white">Back to Home</Link>
      </div>
    </main>
  );
}
